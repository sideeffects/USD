...
void HdxPrman_InteractiveContext::Begin(HdRenderDelegate *renderDelegate)
{
    ...
    // Options
    {
        ...
        // XXX: In RenderMan 22 resizing the viewport wasn't supported 
        ...
        // TODO: Re-evaluate with RenderMan 23 resolution edit support.
        ...
        // set pixel aspect ratio from render settings
        const VtValue parVal = renderDelegate->GetRenderSetting(
            HdPrmanRenderSettingsTokens->pixelAspectRatio);

        if (parVal.IsHolding<float>()) {
            pixelAspectRatio = parVal.UncheckedGet<float>();
        } else {
    ...
    riley::RenderTargetId rtid = riley::RenderTargetId::k_InvalidId;
    {
        ...
        uint32_t renderTargetFormat[3] = 
            {uint32_t(resolution[0]), uint32_t(resolution[1]), 1};
